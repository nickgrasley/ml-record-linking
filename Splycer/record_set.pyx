...
class RecordDB(
    ...
    def get_records(self, uids):
        ...
        while table_exists:
            ...
            table_exists = \
                self.cursor.execute(
                    f"if object_id('dbo.temp_idx{i}', 'U') is not null select 1 else select 0").fetchone()[
                    0]

        self.cursor.execute(f'create table temp_idx{i} ([index_] int not null primary key)')  # create temporary table
        self.cursor.executemany(f'INSERT INTO temp_idx{i} VALUES (?)',
                                indices)  # insert target people indices into new table

        # merge temporary table indices onto record_set table
        if self.var_list is None:
            data = pd.read_sql(
                f"select * from {self.table_name} right join temp_idx{i} on (temp_idx{i}.[index_]={self.table_name}.[index]) {self.extra_joins}",
                self.conn)
        else:
            data = pd.read_sql(
                f"select {self.var_list} from {self.table_name} right join temp_idx{i} on (temp_idx{i}.[index_]={self.table_name}.[index]) {self.extra_joins}",
                self.conn)

        self.cursor.execute(f'drop table temp_idx{i}')  # delete temp table after merge complete
        self.conn.commit()
        uids = pd.Series(uids)
        uids = uids[uids.isin(data['index']).tolist()].tolist()
