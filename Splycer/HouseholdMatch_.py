...
class HouseholdByAnchors():
    ...
    """
    def __init__(self, df1, df2, anchors, year1, year2, cw1, cw2, iterlist_step_size=None, anchors_are_hh=True):
        """ Parameters:
        ...
        if 'household' in df1.columns:
            df1 = df1.rename(columns={'household':'household'+self.year1})
        if 'household' in df2.columns:
            df2 = df2.rename(columns={'household':'household'+self.year2})self.df1 = df1
        self.df2 = df2
        self.cw1 = cw1
        self.cw2 = cw2
        ...
        if anchors_are_hh:
            ...
            if self.index1 not in anchors.columns:
                anchors = pd.merge(anchors, self.cw1, on=self.ark1)
            if self.index2 not in anchors.columns:
        ...
        if iterlist_step_size is None:
            ...
            iterlist = list(range(0, anchors.shape[0], iterlist_step_size))+[anchors.shape[0]]
    ...
    def predict(self, dest_folder):
        ...
        for start,stop in self.iterlist:
            ...
            hhd1 = self.make_hhdict(self.df1[self.df1[self.hh1].isin(hhset1)])
            hhd2 = self.make_hhdict(self.df2[self.df2[self.hh1].isin(hhset2)])
...
class HouseholdByRelPairs():
    ...
    def predict(self, bp, dest_folder):
        """ Runs the entire process and returns a set of matches. """
