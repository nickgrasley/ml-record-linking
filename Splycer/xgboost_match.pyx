...
class XGBoostMatch(LinkerBase):
    ...
    def get_feature_importances(self):
        """Return a dataframe containing the different types of feature
        importance for the models -- used in save method """
        ...
        with tqdm(total=self.compareset.ncompares) as pbar:
            ...
            with open(outfile, "wb") as f:
                for cand_mat in self.compareset.get_pairs(chunksize=chunksize):
                    ...
                    # The coding block below basically re-aligns comparison data frames if we weren't able to pull
                    # record data from indices of the pairs set. With RecordDB we sometimes run into the problem of
                    # bad indices, and this fix allows us to keep running even if there are bad indices in the batch.
                    # Bad index compares are then uploaded to the log file.
                    ...
                    rec2 = rec2.reset_index(drop=True) #FIXME: apppend bad index compares to logfile
                    ...
                    cand_mat = np.vstack(
                        [rec1[self.recordset1.idx_name].to_numpy(), rec2[self.recordset1.idx_name].to_numpy()]).T

                    # column output of below line of code: index1, index2, link_prob
                    np.savetxt(f, np.concatenate((np.array(cand_mat), preds), axis=1)[:, [0, 1, 3]],
                               fmt="%i %i %1.4f")  # FIXME make saving more clear what's what.
    ...
    def rm_duplicates(self, preds_file):
        ...
        dup = dup.merge(max_prob, how="left", left_on="index1", right_index=True).merge(second_max_prob, how="left",
                                                                                        left_on="index1",
                                                                                        right_index=True)
        ...
        if exists:
            while True:
                ...
                if x == 'Y':
                    ...
                    print('invalid input.')
        if not exists:
            with open(f"{path}/model.xgboost",
                      'wb') as file:
                pkl.dump(self.model, file)
            with open(f"{path}/model_scores.json", 'w') as file:
                file.write(
                    f'''{{"training_time": {self.train_time}, "confusion_mat": {list(self.confusion_mat)},"precision": {self.test_precision}, "recall": {self.test_recall}}}''')
            self.comp_eng.save(f"{path}/fe.json")
            self.get_feature_importances.to_csv(f'{path}/feature_importance.csv', index=False)
