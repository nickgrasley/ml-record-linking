...
def feature_eng(file_name):
    ...
    boolean_match = BooleanMatch(vars_to_match=bool_vars)
    
    '''Calculate distance'''
    distance_vars = [["immigration"], ["birth_year"], ["event_lat", "event_lon"], ["bplace_lat", "bplace_lon"]]
    new_distance_cols = ["immigration_dist", "birth_year_dist", "geodist", "bp_geodist"]
    dist = EuclideanDistance(variables=distance_vars, new_cols=new_distance_cols)
    ...
    drop_names = DropVars(cols_to_drop=["first", "last"], both_years=True)
    ...
    comm_weight = CommonalityWeight(cols=columns_to_weight, comm_cols=weights_cols)
    ...
    vars_to_drop = ["marstat", "birth_year", "immigration", #FIXME reinsert index, ark
                    ...
                    "last_dmetaphone", "first_comm", "last_comm", "bp_comm"]
    final_drop = DropVars(vars_to_drop, both_years=True)
